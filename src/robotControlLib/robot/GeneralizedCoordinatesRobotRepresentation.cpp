...
//computes the Gravity Vector for one rigid body
void GeneralizedCoordinatesRobotRepresentation::computeGravityForRB(RobotRB* rb, dVector &g){
    Matrix dpdq;
    compute_dpdq(P3D(0, 0, 0), rb, dpdq);
    Vector3d gravity{0,-9.8,0};
    g += rb->rbProps.mass * dpdq.transpose() * gravity;
}

//computes the Gravity Vector for the whole robot
void GeneralizedCoordinatesRobotRepresentation::computeGravityForcesTerm(dVector &g){

    g.resize(q.size());
    computeGravityForRB(robot->root, g);

    Matrix curMassMatrix;
    for (uint i = 0; i < robot->jointList.size(); ++i){
        computeGravityForRB(robot->jointList[i]->child, g);
    }
}
