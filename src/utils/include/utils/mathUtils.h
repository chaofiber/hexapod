...
//explicit conversion to still exploit the fast eigen operations as much as possible
...
public:
	...
	//this constructor will get called by default every time Vector3d operations are involved
...
// rotate vector using Rodrigues' rotation formulaeuler_to_quaternion
...
//tells us the rotation angle between u and v, given a rotation direction specified by n
...
//checks if the vector x satisfies the linear equation Ax = b. We assume the sparse matrix is symmetric and 
...
inline void getVectorsOrthogonalTo(const V3D& v, V3D& a, V3D& b) {
	...
	if (v[0] * v[0] / (v.squaredNorm()) < 0.5) //if the x component of the current vector is not too large, relatively speaking, then choose x as the main component for a (they won't be aligned perfectly)
		a = V3D(1, 0, 0);
...
inline bool sameRotation(const Quaternion& q1, const Quaternion& q2) {
	//we know that q and -q represent the same rotation, so take that into account...
...
inline void getOrthogonalVectors(const V3D& vec, V3D& a, V3D& b) {
	//try to choose a vector in the y-z plane, if the z-coordinate is significant enough
	if (vec[0] * vec[0] / (vec.squaredNorm()) < 0.5) //if the x component of the current vector is not too large, relatively speaking, then choose x as the main component for a (they won't be aligned perfectly)
		a = V3D(1, 0, 0);
	else //if the x component of the current vector is large, then the y (or z) components cannot be the only non-zero values, so a is not conlinear with the current vector
		a = V3D(0, 1, 0);
